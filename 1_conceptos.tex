...
Cada una de estas soluciones representa un {\it algoritmo}, es
...
En diciembre de 2008, se lanzó la versión 3.0, cuya versión actual es la
3.2, de febrero de 2011. Sin embargo, debido a que estas versiones
...
una instrucción cuyo nombre es \lstinline!holaMar!. ¿Por qué se ponen
...
Si ahora queremos que la máquina ejecute la instrucción
...
cada invocación, para saludar a quien querramos. Para eso están
...
\ejercicioc{Correr tres veces el programa \lstinline!cuad! con valores
de entrada (3,5), (3,3) y (5,3) respectivamente. ¿Qué sucede en
cada caso?}

\ejercicioc{Insertar instrucciones de depuración que permitan ver el valor
...
\ejercicioc{La salida del programa \lstinline!cuad! es poco
informativa. Escribir un programa \lstinline!nom_cuad! que ponga el
número junto a su cuadrado. Ejecutar el programa nuevo.}

\ejercicioc{Si la salida sigue siendo poco informativa seguir mejorándola hasta
que sea lo suficientemente clara.}

\ejercicioc{Escribir un programa que pregunte al usuario:}
\begin{partes}
  \item su nombre, y luego lo salude.
  \item dos números y luego muestre el producto.
\end{partes}

\ejercicioc{Implementar algoritmos que permitan:}
\begin{partes}
...
\ejercicioc{Mostrar el resultado de ejecutar estos bloques de código en el intérprete de python:}
\begin{partes}
\item \begin{verbatim}
>>> for i in range(5):
        print i * i
...
\ejercicioc{Implementar algoritmos que resuelvan los siguientes problemas:}
\begin{partes}
...
\ejercicioc{Escribir un programa que le pida una palabra al usuario, para luego 
