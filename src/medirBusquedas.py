...
diccionario_funciones, todas las funciones de busquedas a medir deben recibir
dos parametros: el primero la lista donde se realiza la busqueda y el segundo
el elemento a buscar.
El modulo provee dos funciones, una para pruebas de busquedas exitosas y otra
para busquedas no exitosas. Resulta curioso ver que la funcion de busqueda
busConIndex, que utiliza el operador in, presenta mejor perfomance en el caso
de busquedas no exitosas."""
...
def _medir_busquedas(inicio, fin, paso, encontrado):
	"""Funcion que realmente realiza las mediciones """
	resultados={}
	for n in range (inicio, fin, paso):
		...
		for funcion in diccionario_funciones.keys():
			res[funcion]=medirTiempos.medir_tiempos(funcion, [xs, elemento], diccionario_funciones[funcion])
		resultados[n]=res	
	return resultados

def medir_busquedas_exitosas(inicio, fin, paso=1):
	"""Esta funcion mide busquedas exitosas, para lo cual se genera una
	lista ordenada (para poder aplicar busqueda binaria) y se busca el
	ultimo elemento de la lista (Es decir el peor caso).
	...
	Pre:
		inicio	:Tamaño inicial de la lista ordenada, sobre la que
			se realizaran las busquedas. Si es 0, indica la lista
			vacia.
...
def medir_busquedas_no_exitosas(inicio, fin, paso=1):
	"""Esta funcion mide busquedas no exitosas, para lo cual se genera una
...
if __name__ == "__main__" :
	...
	for res in sorted(resultados):
		...
		for funcion in resultados[res].keys():
			print "archivo: %s\nfuncion: %s\n%f\n" % (diccionario_funciones[funcion], funcion, resultados[res][funcion])
		print "\n\n"
